...
export function Header({ lang, onNavigate }: Props) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full flex-col border-b-2 border-[#d6cfc6] bg-[#f4f1ea]/75 backdrop-blur-xl shadow-md transition-all">
      <div className="mx-auto flex w-full lg:border-b-0 border-transparent px-3 py-2 sm:px-6 sm:py-3 xl:px-24">
        <div className="mx-auto flex w-full min-h-[32px] max-w-7xl items-center justify-between sm:min-h-[44px] xl:min-h-[48px]">
          ...
          <nav className="hidden items-center gap-1 lg:flex xl:gap-2">
            <button
              onClick={() => nav('home')}
              className="nav-link font-sleek cursor-pointer rounded-full px-4 py-2 text-[15px] text-[#1a1816] transition-all hover:bg-black/5 hover:text-[#0a2fad] uppercase tracking-wide"
              style={{ fontWeight: 700 }}
            >
              {t('nav.home')}
            </button>

            <div className="h-4 border-l border-black/40 mx-1" />
        ...
        <div className="relative z-20 flex flex-shrink-0 items-center justify-center px-1 sm:px-2">
          <button onClick={() => nav('home')} className="group flex cursor-pointer items-center">
            <h2
              className="font-sleek rounded-sm border-2 border-[#0a2fad] bg-[#f4f1ea] px-2.5 py-0.5 text-lg sm:text-xl md:text-2xl leading-tight text-[#0a2fad] uppercase shadow-[3px_3px_0_0_#0725b0] transition-transform hover:-translate-y-0.5"
              style={{ fontWeight: 700 }}
            >
              P.R.O.O.F
            </h2>
      ...
      >
        <div className="flex w-full flex-col p-6">
          <button
            onClick={() => nav('home')}
            className="font-sleek flex cursor-pointer items-center gap-3 rounded-md px-4 py-3 text-left text-lg text-[#1a1816] transition-all hover:bg-black/5 hover:text-[#0a2fad]"
            style={{ fontWeight: 700 }}
          >
